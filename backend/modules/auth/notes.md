...
from core.dependencies import security  # yaad hai yeh dependencies.py mein hai
...
def logout(
    ...
    # Token ko blacklist karo — 24 ghante ke liye
    redis_client.setex(f"blacklist:{token}", 86400, "true")

    return {"message": "Logout successful!"}

# redis_client.setex(f"blacklist:{token}", 86400, "true")
Key    → "blacklist:eyJhbGc..." (poora token)
Value  → "true"
Time   → 86400 seconds = 24 ghante

24 ghante baad Redis KHUD is entry
ko delete kar dega!